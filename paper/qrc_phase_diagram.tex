...
\caption{Why the bright operating band is meaningful. (a) Bright coordinates are selected often by the validation rule, so they form the operating band. (b--d) The same coordinates are tested after removing damping, removing mixing, or replacing amplitude damping by dephasing. Bright color now means the validation rank gets worse. The loss around the gold band shows that these mechanisms are needed for the useful region.}
\label{fig:evidence}
\end{figure*}
