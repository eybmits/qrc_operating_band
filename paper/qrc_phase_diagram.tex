...
\end{aligned}
\end{equation}
In this expression, $E_{\rm ring}$ is the set of nearest-neighbor edges on the qubit ring, $Z_i$ is the Pauli-$Z$ operator on qubit $i$, $\lambda$ sets the coupling strength, and $\gamma$ sets the damping rate. We vary only $\theta=(\beta,\lambda,\gamma)$; all other gates are frozen and only a linear readout is trained. The three axes therefore correspond directly to input drive, nonlinear mixing, and controlled forgetting.
