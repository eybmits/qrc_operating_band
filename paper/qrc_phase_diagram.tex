...
\textbf{Reproducibility appendix.} \textbf{Package.} Code, checked-in CSV/JSON artifacts, generated figures, and the \texttt{./reproduce.sh} driver are available in the \href{https://github.com/eybmits/qrc_phase_diagram}{\textcolor{repoLinkBlue}{project repository}}. \textbf{Frozen reservoir.} Exact density matrices start from $\rho_0=|0\cdots0\rangle\langle0\cdots0|$ and are carried between time steps without reset; no finite-shot sampling is modeled. For seed $s$, \texttt{default\_rng(s)} draws $\theta_i\sim U[0,2\pi)$ once; $U_{\rm mix}=\bigotimes_iR_x(\theta_i)$ is reused at every step and layer. Each layer applies $U_\lambda U_{\rm mix}$ on ring edges followed by exact local amplitude damping. The readout uses exact $Z_i$ and ring-pair $Z_iZ_j$ expectations after two layers with an unpenalized-intercept ridge readout; frozen angles for seeds 42--61 are stored in \texttt{data/frozen\_rx\_angles.csv}. \textbf{Selection and diagnostics.} The $(\beta,\lambda,\gamma)$ grid and task splits are those in Sec.~II; ridge penalties $\{10^{-8},\ldots,10^3\}$ are selected by validation NMSE only. MC/IPC use an iid $U[-1,1]$ drive of length 900, washout 150, seed 12345, ridge $\alpha=10^{-8}$, and a 70/30 chronological split. MC sums held-out $R^2_+$ over delays 1--10; IPC adds linear delays 1--20, quadratic Legendre delays 1--10, and ten cross-delay products, clipping negative $R^2$ to zero. All CIs use 50,000 bootstrap resamples with seed 20260523.
