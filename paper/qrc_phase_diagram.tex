...
\caption{Why the operating band is meaningful. (a) Bright coordinates are selected often by the validation rule, so they form the operating band. (b--d) The same control-space slice is recomputed after removing damping, removing mixing, or replacing amplitude damping by dephasing. These three panels intentionally use the reversed color direction: dark color means larger rank loss, i.e., worse validation rank after the mechanism is changed. The damage concentrates around the gold band, showing that the useful region depends on these mechanisms within this reservoir family.}
\label{fig:evidence}
\end{figure*}
