...
def robustness_row(
    label: str,
    df: pd.DataFrame,
    band_keys: set[Tuple[float, ...]],
    base_keys: set[Tuple[float, ...]],
    all_points: pd.DataFrame,
) -> Dict[str, object]:
    band_points = pd.DataFrame(list(band_keys), columns=KEYS) if band_keys else pd.DataFrame(columns=KEYS)
    comps = connected_components(band_points, all_points)
    band_mask = df[KEYS].apply(lambda r: tuple(float(r[k]) for k in KEYS) in band_keys, axis=1)
    held = (
        df[band_mask]
        .groupby(["task", "seed"])
        .agg(holdout_rank=("test_rank_pct", "mean"), validation_rank=("val_rank_pct", "mean"))
        .reset_index()
    )
    denom = len(base_keys | band_keys)
    overlap = len(base_keys & band_keys) / denom if denom else float("nan")
    return {
        "setting": label,
        "band_exists": bool(len(band_keys) > 0),
        "band_size": int(len(band_keys)),
        "components": int(comps["components"]),
        "largest_component": int(comps["largest_component"]),
        "mean_heldout_rank": float(held["holdout_rank"].mean()) if len(held) else float("nan"),
        "mean_validation_rank": float(held["validation_rank"].mean()) if len(held) else float("nan"),
        "band_overlap_with_base": float(overlap),
        "n_task_seed": int(len(held)),
    }
...
def main() -> None:
    ...
    robustness_rows: List[Dict[str, object]] = [
        robustness_row("4q, 2 layers, Z+ZZ", q, primary_keys, primary_keys, all_points)
    ]
    robustness_path = DATA / "qrc_architecture_robustness_grid.csv"
    if not robustness_path.exists():
        raise FileNotFoundError(f"Missing {robustness_path}; run scripts/run_qrc_phase_grid.py first.")
    variant = add_ranks(pd.read_csv(robustness_path))
    variant_points = variant[KEYS].drop_duplicates().reset_index(drop=True)
    variant_primary = band_table(variant, PRIMARY_P, PRIMARY_Q)
    variant_keys = key_tuples(variant_primary[variant_primary["in_band"]])
    robustness_rows.append(
        robustness_row("4q, 3 layers, Z+ZZ", variant, variant_keys, primary_keys, variant_points)
    )
    robustness = pd.DataFrame(robustness_rows)
    robustness.to_csv(DATA / "phase_map_architecture_robustness.csv", index=False)
    ...
    stats = {
        ...
        "architecture_robustness": robustness_rows,
    ...
    for row in robustness_rows:
        exists = "yes" if row["band_exists"] else "no"
        robustness_tex_rows.append(
            rf"{row['setting']} & {exists} & {fmt(row['mean_heldout_rank'])} & {fmt(row['band_overlap_with_base'], 2)}\\"
        )
    ...
    macros = [
        ...
        macro("PhaseRobustnessRows", "\n".join(robustness_tex_rows)),
