...
fig.canvas.draw()
right_box = axes[-1].get_position()
cax = fig.add_axes([right_box.x1 + 0.0075, right_box.y0, 0.0065, right_box.height])
cbar = fig.colorbar(im, cax=cax)
cbar.outline.set_visible(False)
cbar.set_ticks([0.2, 0.4, 0.6, 0.8])
...
cax = fig.add_axes([right_box.x1 + 0.010, right_box.y0, 0.010, right_box.height])
cbar = fig.colorbar(delta_im, cax=cax)
cbar.outline.set_visible(False)
cbar.set_ticks([0.0, 0.15, 0.30, 0.45])
cbar.ax.tick_params(labelsize=5.4, length=1.8, width=0.5, pad=1.5)
cbar.set_label("rank loss", fontsize=6.0, labelpad=3)
