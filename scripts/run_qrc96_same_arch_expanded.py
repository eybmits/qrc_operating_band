#!/usr/bin/env python3
"""Run the expanded same-architecture QRC96 grid for task-wise ESN comparison."""
...
BETA_FRACS = np.array([0.005, 0.010, 0.015, 0.020, 0.030], dtype=float)
LAMBDA_FRACS = np.array([0.035, 0.050, 0.075, 0.100, 0.125], dtype=float)
GAMMAS = np.array([0.050, 0.080, 0.100, 0.120, 0.160], dtype=float)
...
def run(output: Path, metadata_output: Path, task_profile: str, overwrite: bool) -> None:
    if output.exists() and not overwrite:
        if existing_complete(output):
            print(f"{output} already contains the complete 5x5x5 QRC96 grid; leaving it unchanged.")
            return
                ...
                for lambda_frac in LAMBDA_FRACS:
                    for gamma in GAMMAS:
                        ...
                        if done % 100 == 0 or done == total:
                            ...
                            print(f"qrc96 same-arch expanded {done}/{total} task-configs, elapsed {elapsed:.1f}s", flush=True)
    ...
    metadata = {
        "description": "Expanded same-architecture QRC96 grid for validation-only shared and task-wise ESN100 comparisons.",
        "architecture_lock": {
            "n": 4,
            "layers": 2,
            "topology": "ring",
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "data" / "qrc96_same_arch_expanded_grid.csv")
    parser.add_argument(
        "--metadata-output", type=Path, default=ROOT / "data" / "qrc96_same_arch_expanded_metadata.json"
    )
