...
- `qrc_seed_ensemble_grid.csv`: main QRC16 phase-map grid across four tasks, twenty seeds, and the coarse `(beta, lambda, gamma)` grid; 19,600 rows.
- `qrc_phase_grid_metadata.json`: task splits, grid values, seeds, ridge grid, base architecture, and nearby-depth robustness setting for the canonical phase-grid run.
- `qrc_architecture_robustness_grid.csv`: nearby architecture variant using four qubits, three layers, the same `Z+ZZ` readout, and the same 20-seed protocol; 19,600 rows.
- `qrc_phase_ablation_slice_grid.csv`: focused QRC-only mechanism ablation sweep at the central `gamma=0.12` slice; 27,440 rows.
...
- `frozen_rx_angles.csv`: exact per-seed frozen `Rx` mixer angles for seeds 42--61 in the main QRC16 simulations.
...
- `phase_map_architecture_robustness.csv`: compact architecture robustness table comparing the base 4q/2-layer band with the 4q/3-layer band.
...
- `paper/gfx/fig3_memory_capacity_screens.{png,pdf}`: QRC-only diagnostic figure.
